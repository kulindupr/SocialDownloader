...
import { Facebook, Link2, Loader2, Download, AlertCircle, CheckCircle, X, Zap, Shield, Music, Video } from 'lucide-react';
import { fetchVideoInfo, downloadVideo, downloadFacebookAudio } from '../services/api';
...
const FacebookPage = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const sanitizedTitle = (videoInfo.title || 'facebook-video')
        .replace(/[^\w\s-]/g, '')
        .replace(/\s+/g, '_')
        .substring(0, 100);
      ...
      if (downloadType === 'audio') {
        ...
        await downloadFacebookAudio(url, filename, (prog) => setProgress(prog));
      } else {
        if (!selectedFormat) return;
        const filename = `${sanitizedTitle}_${selectedFormat.height}p.mp4`;
        await downloadVideo(
          url,
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Download videos from Facebook in HD quality
            </p>
          </motion.div>
            ...
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 rounded-xl bg-gray-100 dark:bg-gray-800 border-0 focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white placeholder-gray-400 transition-all"
                />
              </div>
              <motion.button
                ...
                className="px-6 py-4 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg cursor-pointer"
              >
                {loading ? (
            ...
            {videoInfo && (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={`py-3 px-4 rounded-xl text-sm font-medium transition-all cursor-pointer flex items-center justify-center gap-2 ${
                          downloadType === 'video'
                  ...
                  {/* Quality Selector - Only show for video */}
                  ...
                  <motion.button
                    ...
                    className="w-full py-4 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg cursor-pointer"
                  >
                    {downloading ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
                      ...
                      <>
                        {downloadType === 'audio' ? <Music className="w-5 h-5" /> : <Download className="w-5 h-5" />}
                        <span>
                          {downloadType === 'audio' 
                            ? 'Download MP3' 
